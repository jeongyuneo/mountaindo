package com.hanssarang.backend.common.domain;

public class ErrorMessage {

    public static final String FAIL_TO_SIGNUP = "가입에 실패했습니다.";
    public static final String FAIL_TO_CREATE_SURVEY = "사전 설문조사 저장에 실패했습니다.";
    public static final String FAIL_TO_UPDATE_MEMBER = "회원정보 수정에 실패하였습니다.";
    public static final String FAIL_TO_UPDATE_PASSWORD = "비밀번호 수정에 실패하였습니다.";
    public static final String FAIL_TO_DELETE_MEMBER = "회원탈퇴에 실패하였습니다.";
    public static final String FAIL_TO_SAVE_FILE = "파일 저장에 실패했습니다.";
    public static final String FAIL_TO_GET_MOUNTAINS = "산 목록 조회에 실패했습니다.";
    public static final String FAIL_TO_SEARCH_MOUNTAIN = "산 검색에 실패했습니다.";
    public static final String FAIL_TO_SEARCH_TRAIL = "등산로 검색에 실패했습니다.";
    public static final String FAIL_TO_SEARCH_MOUNTAIN_OR_TRAIL = "산 또는 등산로 검색에 실패했습니다.";
    public static final String FAIL_TO_CREATE_HIKING = "등산 정보 저장애 실패했습니다.";

    public static final String NOT_EQUAL_PASSWORD = "비밀번호가 일치하지 않습니다.";
    public static final String NOT_FOUND_MEMBER = "사용자를 찾을 수 없습니다.";
    public static final String NOT_FOUND_HIKING = "등산 정보를 찾을 수 없습니다.";
    public static final String NOT_FOUND_MOUNTAIN = "해당 산을 찾을 수 없습니다.";
    public static final String NOT_FOUND_TRAIL = "해당 등산로를 찾을 수 없습니다.";

    public static final String NOT_EQUAL_VALIDATION_TOKEN = "이메일 인증번호가 일치하지 않습니다.";

    public static final String DUPLICATED_EMAIL = "중복된 이메일이 있습니다.";
    public static final String DUPLICATED_NICKNAME = "중복된 닉네임이 있습니다.";

    public static final String WRONG_PATH = "잘못된 경로입니다.";
    public static final String WRONG_CONTENT_TYPE = "잘못된 확장자입니다.";
}
