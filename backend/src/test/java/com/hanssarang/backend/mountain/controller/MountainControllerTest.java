...
class MountainControllerTest extends ApiDocument {
    ...
    void getMountainsSuccess() throws Exception {
        // given
        willReturn(mountainListResponses).given(mountainService).getMountains(NAME_ORDER, anyInt());
        // when
    ...
    void getMountainsFail() throws Exception {
        // given
        willThrow(new UnexpectedRollbackException(FAIL_TO_GET_MOUNTAINS)).given(mountainService).getMountains(NAME_ORDER, anyInt());
        // when
        ResultActions resultActions = 산_목록_조회_요청(NAME_ORDER, PAGE);
        // then
    ...
    private ResultActions 산_검색_요청(String keyword) throws Exception {
